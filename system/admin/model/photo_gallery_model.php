...
/*------------ KATEGÓRIÁK KEZELÉSE ---------------*/
	...
	/**
	 *	Visszaadja a photo_gallery táblából a photo_gallery oszlopot
	 */
	...
	{
		$data['category_name'] = $this->request->get_post('value');
		$id = $this->request->get_post('pk', 'integer');

			// kategórianév ellenőrzés
			$this->query->reset();
			$this->query->set_table(array('photo_category'));
			$this->query->set_columns('COUNT(*)');
			$this->query->set_where('category_name', '=', $data['category_name']);
			$result = $this->query->select();

			if($result[0]['COUNT(*)'] == 1) {
				return array(
					'success' => 'false',
					'msg' => 'Már létezik ilyen nevű kategória!'
				);
			}
